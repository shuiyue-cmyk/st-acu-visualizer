            ...
            const injectApple = () => {
                ...
                const css = `
                    <style id="acu-v2-apple">
                        ...
                        #acu-app-v2 {
                            /* 三档透明度梯度拉开纵深。shell 半透明(0.55)+ 全屏轻 blur 8px
                               = 磨砂但酒馆轮廓可辨(用户实机反馈:0.62+12px 糊成纯色辨不出
                               酒馆,降 blur 提透;8px 磨砂感仍在又透出底下明暗);面板次透、
                               灰块再一档,每层之间透出模糊背景形成层次。 */
                            --acu-bg-0: rgba(244, 244, 247, 0.55) !important;
                        ...
                        /* 全屏 shell:轻 blur 8px 做磨砂底(8px 保留磨砂感但酒馆明暗轮廓可辨,
                           12px 曾把底下糊成纯色——用户实机反馈;8px 远轻于历史卡顿源
                           20-24px)。其余三个全屏遮罩层(dialog-layer/drawer-layer/
                           mobile-nav-layer 是覆盖视口的弹窗遮罩,透明黑底无需 blur)保持 none。 */
